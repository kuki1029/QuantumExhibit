...
export const SimulationQuirks = ({ isLightTheme }) => {
    return (
        ...
        <p className="sPText">
          ...
          are few a of the things that I implemented in my own unique ways:
        </p>
        {/* Damping */}
        <h4>
          Damping (sort of...)
        ...
        <p className="sPText">
            ...
            over time to mimic a real world double pendulum. One could add air resistance or some arbitrary applied force.
            So why didn't I? This makes the already complicated problem into an even more complicated one but still a solvable one. 
            Well, if it is solvable, why didn't I solve that? The main reason I chose not to solve it directly is because
            it would just take me too long and would be an even lengthier derivation. However, I found a simple workaround for this problem. <br /><br />
            The first method I tried was multiplying each iteration of the numerical integration by a coefficient close to 1
            such as 0.999. This would make it so that each iteration would slowly decrease and slow down the pendula. This worked
            for small angles, however, it introduced some very erratic behaviour at larger angles or speeds. The pendula would
            just stop at random angles and move in random directions. This should've been obvious because multiplying each 
            iteration by a fixed constant ruins the whole point of the numerical method (Runge-Kutta 4). <br /><br />
            Then I thought of just subtracting some amount from the speed of each pendulum after integration. Again, this led to erratic behaviour
            which didn't make sense and didn't relate to what one would expect from a real world double pendulum. <br /><br />
            The solution to this problem was to modify the equations for the acceleration. I added a subtractive term to the <MathJax inline>{"\\( \\ddot{\\theta} \\)"}</MathJax>  calculations in the form of <MathJax inline>{"\\( -a \\omega \\)"}</MathJax>. Where <MathJax inline>{"\\( a  \\)"}</MathJax> is 
            ...
            with the magnitude of the values and thus led to erratic behaviour. 
        </p>
        {/* Graph Plotting */}
        <h4>
          Plotly Graphs
        ...
        <p className="sPText">
            ...
            that are greater than <MathJax inline>{"\\( 2\\pi \\)"}</MathJax> or less than zero. These angles still 
            relate to values within that range and it doesn't make a difference in the math but it does make a 
            difference when graphing. <br /><br />
            ...
            easy fix, just find the modulus and make sure the angles are in the required domain. <br /><br />
            The more complicated problem came when passing this data into PlotlyJS. Let us say the value to plot was 
            <MathJax inline>{"\\( (\\pi, 3\\pi) \\)"}</MathJax>. The second value is outside our domain so we find the equivalent angle and get <MathJax inline>{"\\( \\pi \\)"}</MathJax>. When plotting this new value in Plotly, we got this weird behaviour: 
            <img
            ...
            just skip this problem, but that was not what I had in mind for this. <br /><br />
            To fix this, I had to add a new trace to the plot every time I had to fix the domain. This means that instead of 
            ...
            and playing around with the Plotly library. I definitely learned a lot about the library and coming up with unique solutions. 
        </p>
        {/* Checking Accuracy */}
        <h4>
          Accuracy
        ...
        <p className="sPText">
            ...
            be sure that my simulation is actually correct? <br /><br />
            ...
            Otherwise, I'd be violating one of the fundamental laws of the universe. <br /><br />
