...
export default class DoublePendulumAnimation {
  ...
  constructor(app, originX, originY, defaultLen1, defaultLen2) {
    ...
    this.pend = new DoublePendulumData(
      DefaultDoublePend.mass1,
      DefaultDoublePend.mass2,
      defaultLen1,
      defaultLen2
    );
  ...
  displayEnergyText() {
    if (this.showEnergy) {
      ...
      this.energyText.text = `Kinetic: ${
        Math.round(ke * 100) / 100
      }\nPotential: ${Math.round(pe * 100) / 100}\nTotal: ${
        Math.round((ke + pe) * 100) / 100
      }`;
    } else {
  ...
  drawTraces(x1, y1, x2, y2) {
    if (this.showTrace1) {
      this.firstTrace
        .lineTo(this.originX + x1, this.originY + y1)
    ...
    if (this.showTrace2) {
      this.secondTrace
        .lineTo(this.originX + x1 + x2, this.originY + y1 + y2)
  ...
  dragFirstpendulum(x, y) {
    ...
    if (this.setAngle1Slider && this.setLen1Slider) {
  ...
  dragSecondpendulum(x, y) {
    this.drawRopes(
      ...
      this.currentPosition.y1,
      x - this.currentPosition.x1,
      y - this.currentPosition.y1
    );
    const len = this.calculateLength(
      x - this.currentPosition.x1,
      y - this.currentPosition.y1
    );
    ...
    if (this.setAngle2Slider && this.setLen2Slider) {
  ...
  initTraces() {
    const { x1, y1, x2, y2 } = this.pend.calculateNextPos();
    this.firstTrace
      .moveTo(x1 + this.originX, y1 + this.originY + this.defaultLen1)
      .stroke({ width: 2, color: this.pendulumColor });
    this.firstTrace.clear();
    this.secondTrace
      .moveTo(x2 + this.originX, y2 + this.originY + this.defaultLen1)
      .stroke({ width: 2, color: this.pivotColor });
    this.secondTrace.clear();
  }
   ...
   */
  drawPendulums() {
    this.pendulum1.circle(
      this.originX,
      this.originY,
      DefaultDoublePend.pendulumSize
    );
    this.pendulum1.fill(this.pendulumColor);

    this.pendulum2.circle(
      this.originX,
      this.originY,
      DefaultDoublePend.pendulumSize
    );
